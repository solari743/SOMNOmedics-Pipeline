# ===============================================================
# CONTINUOUS SLEEP BOWL SKEW PLOT (With Sleep State Legend)
# ===============================================================
# 1. Cleans accelerometer data (clip, detrend, low-pass)
# 2. Computes "Bowl Skew" metric per 30s epoch:
#    (mean of upper quartile - mean of lower quartile) / std
# 3. Aligns with sleep file and isolates Sleep periods
# 4. Plots continuous timeline of sleep data:
#    - Color-coded shaded spans = Sleep segments by state
...
def bowl_skew(x):
    ...
    q=int(len(x)*0.25)
    low=x.nsmallest(q).mean(); high=x.nlargest(q).mean()
    std=x.std()
    return np.nan if std==0 else (high-low)/std
...
for state, color in sleep_colors.items():
    ...
    # group continuous state periods separated by >40s
...
legend_elements.append(Patch(facecolor='magenta', alpha=0.5, label='Bowl Skew'))
ax.legend(handles=legend_elements, loc='upper right')

ax.set_title("Continuous Bowl Skew — Sleep States Highlighted", fontsize=14, weight='bold')
ax.set_xlabel("Time (HH:MM:SS)")
ax.set_ylabel("Bowl Skew (unitless)")
ax.grid(True)
