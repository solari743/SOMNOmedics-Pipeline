# ===============================================================
# CONTINUOUS SLEEP MAGNITUDE PLOT (With Sleep State Legend)
# ===============================================================
# 1. Cleans accelerometer data (clip, detrend, low-pass)
# 2. Computes mean acceleration magnitude per 30s epoch
...
# === 5. COMPUTE MEAN MAGNITUDE (30s Epochs) ===
mag_30s = acc_df['mag'].resample('30s').mean().dropna()
...
# Plot mean magnitude over time
ax.plot(mag_30s.index, mag_30s.values, color='magenta', lw=1.5, label='Mean Acceleration Magnitude (m/s²)')
...
# Create custom legend for sleep states
legend_elements = [Patch(facecolor=color, alpha=0.25, label=state.upper()) for state, color in sleep_colors.items()]
legend_elements.append(Patch(facecolor='magenta', alpha=0.5, label='Mean Magnitude'))
ax.legend(handles=legend_elements, loc='upper right')

ax.set_title("Continuous Mean Acceleration Magnitude — Sleep States Highlighted", fontsize=14, weight='bold')
...
# === 8. SUMMARY ===
total_duration = (mag_30s.index[-1] - mag_30s.index[0])
print("\n--- Sleep Magnitude Summary ---")
