# ===============================================================
# CONTINUOUS SLEEP ZCR PLOT (Interactive Slider + Sleep Legend)
# ===============================================================
# 1. Parses sleep states and aligns with accelerometer data
# 2. Computes rolling Zero-Crossing Rate (ZCR)
# 3. Uses 30s epochs with interactive slider navigation
# 4. Displays color-coded sleep-state spans (NREM, REM, Transitional)
# ===============================================================
...
# --- Focus only on sleep states ---
sleep_states = ['nrem', 'rem', 'transitional']
sleep_colors = {'nrem': '#1f77b4', 'rem': '#2ca02c', 'transitional': '#9467bd'}
sleep_only_df = sleep_df[sleep_df['state'].str.lower().isin(sleep_states)].copy()
...
sig = acc_df['ax']  # or use combined magnitude if preferred
...
# === 5. PLOT INTERACTIVE SLIDER VIEW ===
...
def update(i):
    ...
    ax.set_title(f"Sleep State + Zero-Crossing Rate ({start.strftime('%H:%M:%S')})")
    ax.set_xlabel("Time")
    ax.set_ylabel("ZCR (crossings per sample)")
    ax.grid(True)

    # highlight sleep-state spans
    for state, color in sleep_colors.items():
        state_df = sleep_only_df[sleep_only_df['state'].str.lower() == state]
        for s, e in zip(state_df.index, state_df.index.to_series().shift(-1)):
            if pd.isna(e): 
                continue
            if s < end and e > start:
                ax.axvspan(max(s, start), min(e, end), color=color, alpha=0.3)
                ax.axvline(s, color='black', linestyle='--', lw=0.8)

    # plot continuous ZCR
    mask = (zcr_series.index >= start) & (zcr_series.index <= end)
    ax.plot(zcr_series.index[mask], zcr_series[mask], color='magenta', lw=1.3)

    # add legend
    legend_elements = [Patch(facecolor=color, alpha=0.3, label=state.upper()) for state, color in sleep_colors.items()]
    legend_elements.append(Patch(facecolor='magenta', alpha=0.6, label='ZCR'))
