# ===============================================================
# CONTINUOUS SLEEP VELOCITY PLOT (With Sleep State Legend)
# ===============================================================
...
# 4. Aligns with sleep file and isolates Sleep periods
# 5. Plots continuous timeline of sleep data:
...
if pd.isna(dt) or dt == 0:
    dt = 1/50  # default 50 Hz

# Integrate acceleration magnitude to estimate velocity
velocity = np.cumsum(acc_df['mag'] * dt)
vel_df = pd.Series(velocity, index=acc_df.index, name='velocity')

# Resample to 30s means
...
# Plot mean velocity
ax.plot(vel_30s.index, vel_30s.values, color='magenta', lw=1.5, label='Mean Velocity (m/s)')
...
legend_elements.append(Patch(facecolor='magenta', alpha=0.5, label='Mean Velocity'))
ax.legend(handles=legend_elements, loc='upper right')

ax.set_title("Continuous Mean Velocity — Sleep States Highlighted", fontsize=14, weight='bold')
