...
# ===============================================================
# 7. FIRST PLOT – FULL TIMELINE (WITH WAKE) - FIXED VERSION
# ===============================================================
fig, ax = plt.subplots(figsize=(15,7))

# Create a forward-filled version of sleep states to eliminate gaps
sleep_df_filled = sleep_df.copy()

# Create a continuous timeline at 1-second resolution covering the CoV data range
...
# Reindex with forward-fill to extend each state until the next one
sleep_df_filled = sleep_df_filled.reindex(continuous_index, method='ffill')

# If there are any NaN values at the start, backfill them
sleep_df_filled['state_norm'] = sleep_df_filled['state_norm'].fillna(method='bfill')

# Now draw state backgrounds with no gaps
